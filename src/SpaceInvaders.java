...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener {
    ...
    public void draw(Graphics g) {
        ...
        g.drawString("Bullets: " + (maxBullets - bulletsUsed), 10, boardHeight - 10);

        if (bulletsEnded && alienCount > 0 && bulletArray.isEmpty()) {
            g.setFont(new Font("Arial", Font.BOLD, 32));
            g.setColor(Color.red);
            g.drawString("Bullets Ended", boardWidth / 2 - 100, boardHeight / 2);
            g.setFont(new Font("Arial", Font.PLAIN, 24));
            g.drawString("Press any key to restart", boardWidth / 2 - 130, boardHeight / 2 + 40);
        }
    }
    ...
    public void createAliens() {
        ...
        maxBullets = (int) Math.ceil(alienCount * 2);
        bulletsUsed = 0;
        bulletsEnded = false;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        move();
        repaint();
        if (gameOver || (bulletsEnded && alienCount > 0 && bulletArray.isEmpty())) {
            gameLoop.stop();
        }
    }
